...
def accuracy_minibatch(prediction,labels):

    labels = labels.float()
    _, preds = torch.max(prediction, 1) # get predicted class 
    ...
    with torch.no_grad(): # do not record computations for computing the gradient
      ...
      for ctr, data in enumerate(dataloader):
          ...
          if criterion is not None:
            ...
            avgloss = ( avgloss*datasize + curloss*labels.shape[0] ) / ( datasize + inputs.shape[0])

          # for computing the accuracy
          acc= accuracy_minibatch(cpuout,labels)

          accuracy =  (  accuracy*datasize + acc*labels.shape[0] ) / ( datasize + inputs.shape[0])
...
def run():
  ...
  ds={
    'trainval': torchvision.datasets.FashionMNIST('./data', train=True, download=True, transform=datatransforms), 
    'test': torchvision.datasets.FashionMNIST('./data', train=False, download=True, transform=datatransforms)  
  }
  numcl = 10
  indims = 784
  ...
  # model
  # model = onelinear(indims,numcl).to(device) # --> accuracy val 0.8173998594284058 accuracy test 0.8092999458312988
  model = areallyoldschoolneuralnet(indims, numcl).to(device) # accuracy val 0.8939999938011169 accuracy test 0.8907999992370605
  #loss 
  ...
  for lr in lrates: # try a few learning rates
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9) # which parameters to optimize during training?
